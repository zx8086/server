...
type RefToPath<T extends string> = T extends `#/${infer Ref}` ? Pipe<Ref, [Strings.Split<'/'>, Tuples.Join<'.'>]> : never;
...
export type OASResponse<TOAS extends OpenAPIDocument, TPath extends keyof TOAS['paths'], TMethod extends keyof TOAS['paths'][TPath]> = {
    [TStatus in keyof OASStatusMap<TOAS, TPath, TMethod>]: TypedResponse<FromSchema<OASJSONResponseSchema<TOAS, TPath, TMethod, TStatus>>, Record<string, string>, TStatus extends StatusCode ? TStatus : TStatus extends 'default' ? NotOkStatusCode : TStatus extends `${number}${number}${number}` ? ToNumber<TStatus> extends StatusCode ? ToNumber<TStatus> : 200 : 200>;
...
export type OASParamMap<TParameters extends {
    ...
    [TIndex in keyof TParameters]: TParameters[TIndex] extends {
        ...
        [TKey in TParamType extends keyof OASParamPropMap ? OASParamPropMap[TParamType] : never]: TParameters[TIndex] extends {
            required: true;
        } ? {
            [TName in TParameters[TIndex]['name']]: TParameters[TIndex] extends {
                schema: JSONSchema;
            } ? FromSchema<TParameters[TIndex]['schema']> : TParameters[TIndex] extends {
                type: string;
            } ? FromSchema<{
...
export type OASClient<TOAS extends OpenAPIDocument> = {
    [TPath in keyof OASPathMap<TOAS>]: {
        [TMethod in keyof OASMethodMap<TOAS, TPath>]: (requestParams?: OASRequestParams<TOAS, TPath, TMethod>, init?: RequestInit) => Promise<OASResponse<TOAS, TPath, TMethod>>;
    };
};
...
type FixJSONSchema<T> = T extends {
    properties: any;
} ? T extends {
    type: 'object';
} ? T & {
...
} ? OASParamMap<OASMethodMap<TOAS, TPath>[TMethod]['parameters']> : {}) & (OASMethodMap<TOAS, TPath>[TMethod] extends {
    requestBody: {
        content: {
            'multipart/form-data': {
                schema: JSONSchema;
